...
`ios/App/App/Assets.xcassets/Splash.imageset/` is generated by
`python3 scripts/make-splash.py` (Pillow), then `npx cap sync ios`. It is the app
icon on the classic-xanga gradient inside a dotted frame. The canvas is square
and the storyboard scales it `scaleAspectFill`, so a tall phone crops the sides —
the narrowest is 440/956 of the width (~1257px of 2732) on the Pro Max. Keep
anything that must be seen inside the 1150px centre column the script uses, and
check a new one on both the Pro Max and the SE.
