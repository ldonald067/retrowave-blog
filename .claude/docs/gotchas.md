...
- [2026-08-10 /mobile] `applyDynamicType` clears `font-size` at 1x instead of writing 16px. Writing the base value unconditionally overruled a browser user's own default (a 20px reader got 16px), and skipping the write entirely would have stranded a previous scale-up when the setting came back down. Clearing does both jobs.
