...
- **Reduce Motion from the CLI:** `xcrun simctl spawn <udid> defaults write com.apple.Accessibility ReduceMotionEnabled -bool true`, relaunch, restore to `false`. The tell that WKWebView honoured it: the Winamp progress bar sits at 65%.
