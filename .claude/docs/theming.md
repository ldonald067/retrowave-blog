...
A hex-pair sweep cannot see a translucent overlay. `chapter-chip-count` on an
active chip used a hardcoded `rgba(255,255,255,0.25)` over `--accent-primary`
with a `--card-bg` label — **3.61:1 on classic-xanga (the default theme) and
2.94:1 on cottage-core**, because on a light theme a white wash moves the badge
toward its own text. Fixed with
`color-mix(in srgb, var(--text-body) 25%, var(--accent-primary))`, which moves
away from the label on every theme since `--text-body` is always the opposite
pole from `--card-bg`. Worst case is now 5.68:1.

**Rule:** a translucent or `color-mix` background has to be composited against
its actual backdrop before measuring. Grepping for hardcoded hex will not find
it — this one was `rgba()`, and it was the only such literal in the app besides
a theme-neutral box-shadow.
...
- `--text-title` needs **4.5:1**, not 3:1, wherever it renders a card title: on a phone that is `text-lg`, 18px bold, under WCAG's 18.66px bold cutoff. Measuring it against 3:1 is how myspace-blue passed a sweep at 4.00 (findings 38–39).
