...
| 72 | MED | Confirmation email | `{{ .Data.username }}` printed the raw sign-up metadata, which anyone with the anon key can set to any text and send to any address: phishing copy from our domain. GoTrue escapes markup, not words | Fixed — `hook_before_user_created` (migration `20260923000000`, run 2026-09-24; hook switched on the same day). Verified by calling it: a handle passes; a sentence, object, number, capitals, `support` and 31 characters are refused. Not yet seen on a real sign-up |
| 73 | MED | Rename cooldown | Users could write `username_changed_at` directly (update policy covers every column), or blank their username and pick a new one; either reset the cooldown, and with tombstones allowed unlimited name hoarding | Fixed — migration `20260923000000` run 2026-09-24; the trigger fires on every update and pins the date (checked in prod) |
...
- **2026-09-24** — the 72–75 fixes (`4a084c1`), on the iPhone 17 (signed out)
  and against prod. Findings 76–77, both fixed (76 once the migration ran and the hook was switched on). Held
