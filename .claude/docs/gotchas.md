...
- [2026-09-01 /ios] A deep link to a **not-running** app did not route — `com.retrowave.journal://#/report/<id>` landed on the feed on cold start, twice, while the identical link routed every time when the app was already open. That is the emailed "Review in app" case `getLaunchUrl` exists for. Not diagnosed; `App.tsx`'s `hashchange` listener and `consumeAuthCallback` are both ruled out. See the `/ios` section of `docs/audit/ui-audit-plan.md`.
