...
- **Renaming a chapter republishes its entries.** Moving a post from a private chapter to a new name (`"Therapy"` → `"Therapy 2026"`) drops it out of `private_chapters` and it becomes publicly visible. This is deliberate — a rename is a real content move, and the entry's own `is_private` flag is the control. Since 2026-09-15 the editor confirms it first: `chapterChangeRepublishes()` in `utils/chapterPrivacy.ts` decides, and `PostModal` gates the save on a `ConfirmDialog`. The gate sits **before** `onSave`, because once `updatePost` lands the entry is already public and there is no undo for having been seen.
- **The rename confirmation deliberately stays silent on four cases**, and all four are load-bearing: a case/whitespace variant (the server normalizes, so it is not a rename), an entry saved `is_private` (that flag outranks the chapter rule), a private profile (no public page to appear on), and flipping `is_private` true→false by hand (an explicit toggle the user just operated — confirming it asks twice about a decision already made). Widening any of these turns the dialog into noise people click through.
- [2026-09-15 /frontend] **The chapter padlock in `Sidebar` and `ChapterChips` used raw `privateChapters.includes(ch.chapter)`** while `toggleChapterPrivacy` and `get_public_profile` both matched normalized. A case variant therefore drew an unlocked 📖 on a chapter the server was hiding, and the toggle beside it offered to "make private" a chapter it would actually have made _public_. Both now call `isChapterPrivate()`. Grep for `private_chapters` before adding another read of it — normalized is the only correct comparison anywhere.
...
- [2026-08-28 /frontend] The simulator's tap/swipe space is **440x956 points; screenshots come back at ~2.09x that** (~920px wide). Reading a control's position off the screenshot and passing those numbers to `tap` lands roughly twice as far down and right — on empty space. Divide by ~2.09 first. Two taps on ProfileModal's nav button did nothing this way and looked exactly like a dead button; the wiring was fine.
- [2026-08-28 /frontend] `.marquee` is not a class. `index.css` defines `.marquee-banner` + `.marquee-banner-inner`; PublicProfileView used `.marquee` and so never scrolled or clipped, wrapping onto two static lines instead. Nothing in the component looks wrong — only the screenshot and a grep of the stylesheet show it. Worth grepping any class name you did not just read in `index.css`.
- [2026-08-28 /frontend] Retro icons (`Windows95*`, `Winamp`, `VisualStudioFace`) mark **sections**; `Pepicon` marks **controls**. Both families are used app-wide, so neither is legacy — check which role an icon is playing before swapping it, and give each glyph one meaning: `stars` was doing duty for two unrelated ProfileModal sections and `Windows95MyComputer` is already SettingsModal's `data`.
- [2026-09-01 /ios] A deep link to a **not-running** app was dropped — `getLaunchUrl()` resolves async and set `window.location.hash` in the gap between the initial render reading it and the effect attaching the `hashchange` listener, so the event fired into nothing. Fixed in `0a3db5c` by re-reading the hash once inside that effect. If you add another hash-driven route, read it at effect time too, not just in `useState`.
- [2026-09-01 /ios] Re-issuing the **same** deep-link hash is a no-op: `routeDeepLink` returns early when `window.location.hash` already equals it, by design, so a second `simctl openurl` with the same URL does nothing and looks like a dead link. Vary the id.
- [2026-09-01 /mobile] Check **which** simulator is booted and **which build** it is running before auditing on it. A second sim carrying a build from several commits back showed `About Me` and `Current Mood:` still Title Case, and its nav as three identical filled squares — all fixed already. `xcrun simctl install <udid> <app>` over a running install keeps the container, so the signed-in session survives.
- [2026-09-02 /mobile] **Simulator tap delivery is roughly 1-in-3 and sometimes lands seconds late.** The same coordinates that work once will silently do nothing the next three times, and a tap you wrote off as missed can register later — a chapter-privacy toggle flipped after I had already concluded it had not. Verify state from a screenshot after every tap, never infer it from "the tap returned OK", and re-read state before assuming your own earlier action failed. This also makes anything time-boxed (a 3s toast, a sub-400ms double tap) effectively undrivable from here.
- [2026-09-02 /mobile] Catching a transient state needs the screenshot armed _before_ the action: `( sleep N; xcrun simctl io <udid> screenshot f.png ) &` then fire the tap. MCP round trips are ~1.5–2s, so bracket 1.6–3.4s. `xcrun simctl io` is much faster than the MCP screenshot and is the right tool once a tap has already landed.
